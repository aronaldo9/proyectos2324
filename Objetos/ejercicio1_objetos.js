const users = [
  ...
  // 1º ejerc : Sacar y guardar en un array todos los usernames de los usuarios
  // 2º ejerc : obtener el firstname y lastname de todos los usuarios cuyo email no sea .com. 
  // Crear una estructura de tipo map que guarde el firstname y lastname junto con su email
  // V2.0 : hacerlo en una función 



  // 1º Ejercicio
//  const { login : { username : names} } = users;
//  console.log(names);

//  const usernames = misUsuarios => misUsuarios.map(usuario => usuario.login.username);
//  console.log(usernames(users));
 ...
 const first_lastnames = array => array.filter(elemento => !elemento.email.includes(".com")).reduce((acc, user) => {
  ...
  acc.set(`${firstname} ${lastname}`, email);
  return acc;
}, new Map());
